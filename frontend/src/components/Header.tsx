...
function CartIcon(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 24 24" width="18" height="18" {...props}>
      <path d="M7 4h-2l-1 2h2l3.6 7.59-1.35 2.45A1.99 1.99 0 0 0 10 19h9v-2h-8.42a.25.25 0 0 1-.22-.37L11.1 14h6.45a2 2 0 0 0 1.86-1.27L22 7H7.42l-.7-1.4A1 1 0 0 0 5.8 5H3" fill="currentColor"/>
...
function UserIcon(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 24 24" width="18" height="18" {...props}>
      <path d="M12 12a5 5 0 1 0-5-5 5 5 0 0 0 5 5Zm0 2c-5 0-9 2.5-9 6v1h18v-1c0-3.5-4-6-9-6Z" fill="currentColor"/>
    </svg>
  );
}
...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-black/5 bg-white/90 text-black backdrop-blur supports-[backdrop-filter]:bg-white/70">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4">
        ...
        <nav className="hidden items-center gap-10 text-sm font-semibold md:flex">
          {nav.map((n) => (
            <NavLink
              key={n.label}
              to={n.to}
              className={({ isActive }) => `${linkBase} ${isActive ? activeUnderline : ""}`}
              end={n.to === "/"}
            >
              {n.label}
            </NavLink>
        ...
        <div className="flex items-center gap-4 text-sm">
          <Link to="/cart" className="hidden md:flex items-center gap-2 hover:opacity-70">
            <span>Cart ({count})</span>
            <CartIcon />
          </Link>
          <Link to="/profile" className="hidden md:flex items-center gap-2 hover:opacity-70">
            <span>Profile</span>
            <UserIcon />
          </Link>
      ...
      >
        ...
        {/* Mobile quick actions */}
          <div className="mt-2 flex gap-4 border-t border-black/5 pt-3 text-sm">
            <Link to="/cart" onClick={() => setOpen(false)} className="flex items-center gap-2 hover:opacity-70">
              <CartIcon /> <span>Cart ({count})</span>
            </Link>
            <Link to="/profile" onClick={() => setOpen(false)} className="flex items-center gap-2 hover:opacity-70">
